...
class SettingController extends Controller
{
    ...
    public function system_settings_logo_save(Request $request)
    {
        ...
        $darkLogoPath = $this->handleFileUpload($request, 'dark_logo');
        $lightLogoPath = $this->handleFileUpload($request, 'light_logo');
        $faviconPath = $this->handleFileUpload($request, 'favicon');

        if ($darkLogoPath) {
            Setting::where('type', 'system_dark_logo')->update(['description' => $darkLogoPath]);
        }

        if ($lightLogoPath) {
            Setting::where('type', 'system_light_logo')->update(['description' => $lightLogoPath]);
        }

        if ($faviconPath) {
            Setting::where('type', 'system_fav_icon')->update(['description' => $faviconPath]);
        }
