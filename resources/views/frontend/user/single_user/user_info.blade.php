    ...
    <div class="widget page-widget ac_control_we">
        <div class="user_ac">
            ...
            @if ($friend > 0)
                    ...
                    <form class="ajaxForm" action="{{ route('profile.accept_friend_request') }}" method="post">
                        ...
                        @if ($friendAccepted->value('requester') == auth()->user()->id)
                            ...
                            <a href="javascript:void(0)"
                                ...
                                class="btn common_btn px-3" data-bs-toggle="tooltip" data-bs-placement="top"
                        ...
                        @elseif($friendAccepted->value('receiver') == auth()->user()->id)
                            ...
                            <button type="submit" id="friendRequestConfirmBtn{{ $user_data->id }}"
                                class="btn common_btn px-4 no-processing no-uploading" data-bs-toggle="tooltip"
                                data-bs-placement="top" title="{{ get_phrase('Accept Connect Request') }}">
                                {{ get_phrase('Accept') }}
                            </button>
                        @else
                        ...
                        <button type="button" id="friendRequestAcceptedBtn{{ $user_data->id }}"
                            class="btn common_btn d-hidden ac_btn px-4" data-bs-toggle="tooltip" data-bs-placement="top"
                ...
                @endif
            @else
                <a href="javascript:void(0)" onclick="ajaxAction('<?php echo route('user.friend', $user_data->id); ?>')"
                    class="btn common_btn ac_btn"><i class="fa-solid fa-plus"></i> {{ get_phrase('Connect') }} </a>
            @endif
...
<aside class="sidebar ">
    <div class="widget intro-widget">
        ...
        <form class="ajaxForm d-hidden edit-bio-form"
            action="{{ route('profile.about', ['action_type' => 'update']) }}" method="post">
            @CSRF
            <div class="mb-3">
    ...
    <div class="widget" id="my-profile-info">
        ...
        <ul>
            ...
            <li>
                ...
                <span>{{ get_phrase('Joined') }}
                    <strong>{{ date_formatter($user_data->created_at, 1) }}</strong></span>
            </li>
        </ul>
    </div>
    <div class="widget">
        ...
        <div id="sidebarPhotoAndVideos" class="row row-cols-3 g-1 mt-3">
            @foreach ($media_files as $media_file)
                ...
                @else
                    <div class="single-item-countable col"><a
                            href="{{ route('single.post', $media_file->post_id) }}"><img
                                class="img-thumbnail w-100 user_info_custom_height"
                                src="{{ get_post_image($media_file->file_name, 'optimized') }}" alt=""></a>
                        </li>
                    </div>
                @endif
            @endforeach
        ...
        <a href="{{ route('user.photos', [$user_data->id, 'identifire' => $identifires]) }}"
            class="btn common_btn mt-3 d-block mx-auto">{{ get_phrase('See More') }}</a>
    </div>
    <!--  Widget End -->
        ...
        @php
            $friends = DB::table('friendships')
                ->where(function ($query) use ($user_data) {
                    $query->where('accepter', $user_data->id)->orWhere('requester', $user_data->id);
                })
                ->where('is_accepted', 1)
            ...
            @foreach ($friends->take(6)->get() as $friend)
                ...
                <div class="col">
                    <a href="{{ route('user.profile.view', $friends_user_data->id) }}" class="friend d-block">
                        <img width="100%" class="h-90"
                            src="{{ get_user_image($friends_user_data->photo, 'optimized') }}" alt="">
                        <h6 class="small">{{ $friends_user_data->name }}</h6>
                    </a>
                </div>
            @endforeach
        </div>
        <a href="{{ route('user.friends', $user_data->id) }}"
            class="btn common_btn mt-3 d-block mx-auto">{{ get_phrase('See More') }}</a>
    </div>
    <!--  Widget End -->
