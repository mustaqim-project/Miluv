            ...
            @if ($message->reciver_id == auth()->user()->id)
                            ...
                            <div class="text-quote mt-0">
                                @if (\Illuminate\Support\Str::contains($message->message, 'http', 'https'))
                                    ...
                                    @if ($explode_data[count($explode_data) - 2] == 'post')
                                        <iframe src="{{ route('custom.shared.post.view', $shared_id) }}?shared=yes"
                                            scrolling="no" class="w-100" onload="resizeIframe(this)"
                                            frameborder="0"></iframe>
                                    @else
                                        <iframe src="{{ route('single.product.iframe', $shared_id) }}?shared=yes"
                                            scrolling="no" class="w-100" onload="resizeIframe(this)"
                                            frameborder="0"></iframe>
                                    @endif
                                    <a class="text-dark ellipsis-line-2" href="{{ $message->message }}"
                ...
                @if (!empty($message->thumbsup) && empty($message->message))
                    <div class="d-flex user-quote position-relative">
                        <img src="{{ get_user_image($user->id, 'optimized') }}" alt="" class="avatar-sm me-2">
                        <div class="chat-react"><img src="{{ asset('assets/frontend/images/like-lg.png') }}"
                                alt=""></div>
                    </div>
                @endif
                            ...
                            @if ($file->file_type == 'image')
                                <div class="quote-box">
                                    <img src="{{ asset('storage/chat/images/' . $file->file_name) }}" alt=""
                                        style="max-width: 50px; max-height: 50px; object-fit: cover; border-radius: 8px;">
                                </div>
                            @else
                                <div class="quote-box">
                                    <video style="max-width: 80px; max-height: 50px; object-fit: cover; border-radius: 8px;" controls>
                    ...
                    <div class="d-flex user-quote user-reply justify-content-end">
                        <div class="quote-react remove-icon-message">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                <li><a class="dropdown-item" href="{{ route('remove.chat', $message->id) }}">
                                        {{ get_phrase('Remove') }}</a>
                                </li>
                            </ul>
                        </div>
                            ...
                            @if (\Illuminate\Support\Str::contains($message->message, 'http', 'https'))
                                ...
                                @else
                                    <iframe src="{{ route('single.product.iframe', $shared_id) }}?shared=yes"
                                        ...
                                        frameborder="0"></iframe>
                                @endif
                                <a href="{{ $message->message }}" class="text-dark ellipsis-line-2"
                                    target="_blank">{!! $message->message !!}</a>
                            @else
                                {!! $message->message !!}
                            @endif
                                    ...
                                    <div class="quote-react remove-icon-message">
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                            <li><a class="dropdown-item"
                                                    href="{{ route('remove.chat', $message->id) }}">
                                                    {{ get_phrase('Remove') }}</a>
                                            </li>
                                        </ul>
                                    </div>
                                    <img class="rounded" src="{{ asset('storage/chat/images/' . $file->file_name) }}"
                                        class="quote_image_box_image" alt="">
                                </div>
                            @else
                                <div class="quote-box d-flex">
                                    <div class="quote-react remove-icon-message">
                ...
                @if (!empty($message->thumbsup) && empty($message->message))
                    <div class="d-flex user-quote user-reply justify-content-end">
                        <div class="d-flex user-quote">
                            ...
                            <div class="chat-react mt-2"><img class="rounded"
                                    src="{{ asset('assets/frontend/images/like-lg.png') }}" alt=""></div>
                        </div>
                    </div>
                @endif
            @endif
